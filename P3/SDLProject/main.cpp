...
void Initialize() {
    ...
    displayWindow = SDL_CreateWindow("Finally, Obamium!", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, 640, 480, SDL_WINDOW_OPENGL);
    ...
    state.player->acceleration = glm::vec3(0, -0.015f, 0);
    state.player->speed = 1.5f;
    state.player->textureID = LoadTexture("Obama_Prism.png");
    ...
    GLuint platformTextureID = LoadTexture("mario_block.png");
    GLuint goalTextureID = LoadTexture("question_block.jpg");
    GLuint fontTextureID = LoadTexture("font1.png");

    state.platforms[0].textureID = platformTextureID;
    state.platforms[0].position = glm::vec3(-2, -3.25f, 0);

    state.platforms[1].textureID = platformTextureID;
    state.platforms[1].position = glm::vec3(-1, -3.25f, 0);

    state.platforms[2].textureID = platformTextureID;
    state.platforms[2].position = glm::vec3(0, -3.25f, 0);
    ...
    state.platforms[5].position = glm::vec3(-5, -3.25f, 0);
    ...
    state.platforms[9].position = glm::vec3(3, 0.25f, 0);

    state.platforms[10].textureID = platformTextureID;
    state.platforms[10].position = glm::vec3(2, 0.25f, 0);


    state.goals = new Entity[GOAL_COUNT];

    state.goals[0].textureID = goalTextureID;
    state.goals[0].position = glm::vec3(-4, -3.25f, 0);

    state.goals[1].textureID = goalTextureID;
    state.goals[1].position = glm::vec3(-3, -3.25f, 0);
...
void Update() {
    ...
    while (deltaTime >= FIXED_TIMESTEP) {
        ...
        if (state.player->collidedTop == true || state.player->collidedBottom == true
            || state.player->collidedLeft == true || state.player->collidedRight == true)
        {
            gameLost = true;
        }
